...
#define UNIX_SOCKET_DIR		"/tmp/.ki2xwin-unix"
#else
...
#define offset(field) XtOffset(IMProtocolWidget, imp.field)
	...
	offset(transport_list), XtRString, (XtPointer)"x" },
#else
    { XtNtransports, XtCTransports, XtRString, sizeof(String),
...
IMProtocolWidget ipw;
{
    Display *dpy = XtDisplay((Widget)ipw);
#ifdef USE_WINIME
    Atom xim_servers = XInternAtom(dpy, "XIM_SERVERS", True);
#else
...
#ifdef USE_WINIME
    if (xim_servers == (Atom)None)
    {
	TRACE(("XIM_SERVERS is null atom.\n"));
...
#ifdef USE_WINIME
TRACE(("Check the XIM_SERVERS property's existance.\n"));
#endif
	if (type != XA_ATOM || format != 32) {
	     ...
	     */
#ifdef USE_WINIME
	    TRACE(("XIM_SERVERS is corrupted (type=%ld, format=%d)\n", type, format));
#else
	...
	{
	    case BadAlloc:
		TRACE(("  XChangeProperty failed: BadAlloc\n"));
		break;
	    case BadAtom:
		TRACE(("  XChangeProperty failed: BadAtom\n"));
		break;
	    case BadMatch:
		TRACE(("  XChangeProperty failed: BadMatch\n"));
		break;
	    case BadValue:
		TRACE(("  XChangeProperty failed: BadValue\n"));
		break;
	    case BadWindow:
		TRACE(("  XChangeProperty failed: BadWindow\n"));
		break;
	    default:
		TRACE(("  XChangeProperty success.\n"));
		break;
	}
#endif
    } else {
...
Widget w;
{
	...
	do {
	    ...
	    *p = '\0';
...
#ifdef USE_WINIME
		int keycode;
		keycode = XKeysymToKeycode(XtDisplay(w), keysym);
		TRACE(("\tkeysym: %08lx (keycode: %02d), modifiers: %04lx, check: %04lx\n",
		       keysym, keycode, mods, chk_mods));
#else
		...
		if (num_keys == 0)
		{
		    g_TriggerKeycode = keycode;
		    g_TriggerModifier = mods & chk_mods;
		}
#endif
		num_keys++;
	    }
