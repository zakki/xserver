...
extern ExtContext *IMIC2Context(IMIC* icp);
extern Window TopLevelWindow (Display * dpy, Window win);
#endif
...
static InputStyle styles[] = {
  ...
  /* // for Motif
    ...
    { XIMPreeditNothing|XIMStatusNothing, IMSTYLE_OVER_THE_SPOT },
    { XIMPreeditArea|XIMStatusArea, IMSTYLE_OVER_THE_SPOT },
    { 0 },
  */
    { XIMPreeditPosition|XIMStatusNothing, IMSTYLE_OVER_THE_SPOT },
    { XIMPreeditPosition|XIMStatusArea, IMSTYLE_OVER_THE_SPOT },
...
{
#ifdef USE_WINIME
    ExtContext *rec = IMIC2Context(icp);
#endif
...
#ifdef USE_WINIME
    if (rec != NULL)
	rec->client_win = icp->common_attr.client;
#endif
    return 0;
}
...
unsigned long mask;
{
    ...
    int preedit;
//#ifdef DEBUG	// comment out	Y.Arai
...
#ifdef FOR_ACS
        /* ACS */
        if (ap->area.width == 0 ) ap->area.width=1;
        if (ap->area.height == 0 ) ap->area.height=1;
#endif
