...
#ifdef USE_WINIME
TRACE(("xBrokenPipe(dispatchException = %d) exit(0)\n", dispatchException));
#endif
	return 0;
    } else {
#ifdef USE_WINIME
TRACE(("xBrokenPipe(dispatchException = %d) exit(1)\n", dispatchException));
#endif
	return 1;
    }
}
...
#ifdef USE_WINIME
TRACE(("\tconn->transport.priv.x.server = %08lx\n", conn->transport.priv.x.server));
#endif
