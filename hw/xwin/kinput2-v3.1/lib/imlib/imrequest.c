...
// 本当はこの仕掛けはXIMプロトコルだけではなくXimp,kinputとも共通にする
static ExtContext *cont_list = (ExtContext *) NULL;
static ExtContext *free_list = (ExtContext *) NULL;
...
#ifdef USE_WINIME
ExtContext *
IMIC2Context(IMIC* icp)
{
    ExtContext *rec = cont_list;
...
void DeleteAllExtContext(void)
{
    ExtContext *rec = cont_list, *tmp;
    while (rec != NULL)
    {
	tmp = rec;
	rec = rec->next;
...
int
GetAllContextConnection(fd_set *fdsSelect)
{
    int iConnect = 0;
    int nMax = 0;
    ExtContext *rec = cont_list;
...
TRACE(("%s()\n", __FUNCTION__));
    while (rec != NULL)
    {
TRACE(("  rec *dpy = 0x%08X\n", rec->dpy));
	if (rec->dpy != NULL)
	{
	    iConnect = ConnectionNumber (rec->dpy);
	    FD_SET (iConnect, fdsSelect);
	    nMax = MAX(nMax, iConnect);
TRACE(("  iConnect = %d\n", iConnect));
	}
	rec = rec->next;
    }
...
//#ifdef USE_WINIME
#if 0
    fRet = XWinIMEGetOpenStatus(XtDisplay(conn->proto_widget), -1, &fOpen);
    if ((fOpen == FALSE) || (fRet == FALSE))
    {
	TRACE(("  WinIME is not opened\n"));
	IMSendBadLength(conn, icp->im->id, icp->id);
	return;
    }
#endif
...
int arglen;
{
    ...
    {
	rec = (ExtContext *) calloc (1, sizeof (ExtContext));
    }
    rec->id = ++icid;
    rec->dpy = XtDisplay(conn->proto_widget);
#endif
...
#ifdef USE_WINIME
#if 1
    XWinIMECreateContext (XtDisplay(conn->proto_widget), &rec->context);
#if 0	// from A.Yamanaka
    // kterm で、最初に +ximで起動してからメニューで入力をONにするとSET_FOCUSがこないのでその対処
    XWinIMESetFocus (XtDisplay(conn->proto_widget), rec->context, True, rec->focus_win);
#endif	// #if 0
#else
    if (fInit == 0)
    {
	...
	XWinIMEGetLastContext (XtDisplay(conn->proto_widget), &rec->context);
#endif
    rec->icp = icp;
    rec->next = cont_list;
    cont_list = rec;
...
int arglen;
{
    ...
    IMSetFocus(icp);
#ifdef USE_WINIME
    rec = IMIC2Context(icp);
    if (rec != NULL)
	XWinIMESetFocus (XtDisplay(conn->proto_widget), rec->context, True, rec->focus_win);
    else
	TRACE(("  ximSetICFocusProc: Can't get ExtContext\n"));
#endif
}
...
int arglen;
{
    ...
    rec = IMIC2Context(icp);
    if (rec != NULL)
	XWinIMESetFocus (XtDisplay(conn->proto_widget), rec->context, False, rec->focus_win);
    else
	TRACE(("  ximSetICFocusProc: Can't get ExtContext\n"));
#endif
}
...
XEvent *ev;
{
	 ...
	 */
#ifdef USE_WINIME
TRACE(("  XIM_SYNC_REPLY from handleForwardedEvent(1)\n"));
#endif
...
{
TRACE(("  dont call IMStopForwarding()\n"));
}
#endif
    /*
     * All the processing is done.  Send XIM_SYNC_REPLY.
	...
	icp->state &= ~IC_FORWARDING;
#ifdef USE_WINIME
TRACE(("  XIM_SYNC_REPLY from handleForwardedEvent(2)\n"));
#endif
	IMSendRequestWithIC(conn, XIM_SYNC_REPLY, 0, icp);
    }
#endif
...
if (kev->type == KeyPress)
	TRACE(("  (type = KeyPress, keycode = %d)\n", kev->keycode));
else if (kev->type == KeyRelease)
	TRACE(("  (type = KeyRelease, keycode = %d)\n", kev->keycode));
else
	TRACE(("  (type = %d)\n", event.type));
...
#ifdef USE_WINIME
TRACE(("  XIM_SYNC_REPLY from ximSyncProc()\n"));
#endif
    IMSendRequestWithIC(conn, XIM_SYNC_REPLY, 0, icp);
}
...
#ifdef USE_WINIME
/*- IMContext2IMIC: WINIMEエクステンションのコンテキスト番号からIMICに変換する -*/
// 暫定
IMIC* IMContext2IMIC (int nContext)
{
    ExtContext *rec = cont_list;

    while (rec != NULL)
    {
	if (rec->context == nContext)
	    return rec->icp;
	rec = rec->next;
    }
