...
TRACE(("ConvCtrl:%s()\n", __FUNCTION__));
TRACE(("*** XtCallCallbackList ***: textcallback\n"));
...
TRACE(("ConvCtrl:%s()\n", __FUNCTION__));
    ...
    if ((ev->time <= LOCALEVENT_MAX) && (ev->serial == 0))
        fDummyEvent = 1;
#endif
...
TRACE(("*** XtCallCallbackList ***: unusedeventcallback\n"));
	...
	CControlEndConversion(w);
TRACE(("*** XtCallCallbackList ***: endcallback, False\n"));
...
TRACE(("*** XtCallCallbackList ***: newtextcallback\n"));
